...
| `KASPI_MERCHANT_ID` | `billing/checkout/route.ts:7` | ID мерчанта Kaspi Pay (казахстанский платёжный провайдер). | Нет явной проверки; см. "требует уточнения" по поводу передачи через CI. |
| `KASPI_API_KEY` | `billing/checkout/route.ts:8` | API-ключ Kaspi Pay. | Нет явной проверки. |
| `KASPI_API_BASE_URL` | `billing/checkout/route.ts:9` | Базовый URL Kaspi API. | Fallback `https://kaspi.kz/online/api`. |
| `KASPI_WEBHOOK_SECRET` | `billing/kaspi-webhook/route.ts:6` | Секрет для проверки подписи вебхука Kaspi. | **Explicit-проверка**: при отсутствии — `500 {result:1, message:'Server misconfiguration'}` (см. `docs/error_codes.md`, раздел 9). |
| `WEBHOOK_DOMAIN` | `billing/webhook/route.ts:199`, `billing/kaspi-webhook/route.ts:128`, `billing/checkout/route.ts:11`, `internal/bot/billing/route.ts:263` и др. (везде — для построения публичных ссылок на чеки/отчёты), а также в `receiptGenerator.ts:46` через `.replace()`, и как источник домена для регистрации Telegram-вебхука в `telegram-bot` | Публичный домен сайта для формирования абсолютных ссылок. | Везде fallback `'https://perricheno.ru'` (кроме telegram-bot, где используется без fallback как один из двух кандидатов — см. раздел 2). |
...
| Переменная | Где используется | Назначение | Обязательность |
|---|---|---|---|
| `NEXT_PUBLIC_WS_URL` | `src/app/[locale]/space/[spaceId]/hooks/useYjsSpace.ts:43` (клиентский React-хук) | URL WebSocket-сервера (`ws-server`) для подключения совместного редактора Yjs из браузера. | Fallback `"ws://localhost:3035"`. **См. "требует уточнения"**: `NEXT_PUBLIC_*`-переменные встраиваются Next.js в бандл на этапе `next build`, а ни `Dockerfile`, ни `docker-compose.yml` не передают её как build-`ARG`/`ENV` в стадию сборки — значит собранный клиентский код, вероятно, всегда содержит fallback-значение независимо от того, что установлено в `.env`/GitHub Secrets на сервере после сборки. |
| `NEXT_PUBLIC_APP_URL` | `src/app/api/agent/generate/route.ts:70`, `src/app/api/space/[id]/share/route.ts:23`, `src/app/api/space/[id]/invite/route.ts:24` | Публичный базовый URL сайта — используется для построения ссылок на расшаренные отчёты/приглашения. | Fallback `'https://perricheno.ru'`. Та же оговорка про build-time инлайнинг, что и выше — переменная нигде не передаётся как build-ARG. |
...
| `WS_SERVER_INTERNAL_URL` | Устанавливается в `docker-compose.yml:20` (`environment:` для `perricheno-site`) | По названию — внутренний адрес `ws-server` для серверных вызовов. | **Нигде не найдено использования в `src/`** — переменная объявлена в топологии деплоя, но код её не читает (клиент подключается к `ws-server` напрямую через `NEXT_PUBLIC_WS_URL`, минуя сервер). Похоже на неиспользуемую/задел на будущее переменную. См. "требует уточнения". |
...
- **`.github/workflows/deploy.yml`** прокидывает секреты GitHub Actions (`secrets.*`) в SSH-сессию на сервере и дописывает/обновляет ими файл `.env` через bash-функцию `update_env` (строки 54-90). В `env:`/`envs:` блоке шага (строки 32-49) явно перечислены и форвардятся: `OPENAI_API_KEY`, `LATEX_COMPILER_URL`, `LATEX_COMPILER_KEY`, `CRYPTOCLOUD_API_KEY`, `CRYPTOCLOUD_SHOP_ID`, `CRYPTOCLOUD_SECRET`, `TELEGRAM_BOT_TOKEN`, `WEBHOOK_DOMAIN`, `WEBHOOK_SECRET`, `WS_JWT_SECRET`, `NEXT_PUBLIC_WS_URL`, `SESSION_SECRET`.
- В скрипте на сервере (строки 80-82) дополнительно вызывается `update_env` для `KASPI_MERCHANT_ID`, `KASPI_API_KEY`, `KASPI_WEBHOOK_SECRET` — **но эти три переменные не объявлены** ни в блоке `env:` (32-44), ни в списке `envs:` (49) самого шага `appleboy/ssh-action`. Поскольку `ssh-action` пробрасывает в удалённую shell-сессию только переменные, перечисленные в `envs:`, значения `$KASPI_MERCHANT_ID`/`$KASPI_API_KEY`/`$KASPI_WEBHOOK_SECRET` внутри `script:` будут пустыми, и (из-за защиты `if [ -n "$value" ]` в `update_env`) строки для Kaspi **не будут записаны/обновлены** в серверный `.env` через этот пайплайн, даже если соответствующие секреты заведены в настройках репозитория GitHub. Комментарий в самом файле (строка 79: "optional - set in GitHub secrets when ready") намекает, что это осознанно неготовая интеграция, а не то, что её забыли — но подтвердить это может только владелец (см. раздел "требует уточнения").
- `MINIO_*` и `SITE_INTERNAL_URL`/`BOT_CONTAINER_URL` (строки 84-90) записываются в `.env` как захардкоженные в самом workflow-файле литералы, а не как секреты — см. раздел 8.
- **`docker-compose.yml`** — каждый сервис либо получает переменные через `env_file: .env` (`perricheno-site`, `ws-server`, `telegram-bot`), либо через явный список в `environment:` (используется и для передачи значений из `.env`-подобного источника, и для захардкоженных внутренних URL вида `http://r-compiler:8000`). У `r-compiler`, `python-compiler`, `research-api`, `pdf-extractor` вообще нет `env_file`/`environment` секции с переменными (кроме служебных для healthcheck) — что согласуется с находкой из разделов 4-7, что эти сервисы почти не читают окружение. `postgres` и `minio` получают креды напрямую в `environment:` захардкоженными литералами (раздел 8).
...
1. **`docker-compose.yml:14` vs `:157-158`** — `DATABASE_URL` для `perricheno-site` захардкожен в `environment:`, дублируя `POSTGRES_USER`/`POSTGRES_PASSWORD`, вместо того чтобы либо собираться из тех же значений через переменные compose (`${POSTGRES_PASSWORD}`), либо читаться из `.env`. При смене пароля Postgres нужно не забыть поправить оба места — это на совести того, кто это поддерживает, стоит уточнить, осознанное ли это решение.
2. **`LATEX_COMPILER_URL` fallback `http://latex-compiler:8000`** (`src/lib/receiptGenerator.ts:110`, `src/app/[locale]/dashboard/actions.ts:107`) — сервиса `latex-compiler` нет в `docker-compose.yml`. Неясно, это удалённый внешний хост (значит fallback в принципе неверный и никогда не должен сработать) или закладка под сервис, который раньше был локальным контейнером и его вывели наружу, не обновив fallback.
3. **`NEXT_PUBLIC_WS_URL` и `NEXT_PUBLIC_APP_URL` не передаются как build-time ARG/ENV** ни в `Dockerfile`, ни в `docker-compose.yml` (сборка — `docker compose build`, без `args:`). Поскольку Next.js встраивает значения `NEXT_PUBLIC_*` на этапе `next build`, а не читает их в рантайме, эти два значения, заданные в серверном `.env`/GitHub Secrets после сборки образа, по всей видимости, не влияют на собранный код — используются захардкоженные в исходниках fallback-и (`ws://localhost:3035`, `https://perricheno.ru`). Стоит подтвердить у владельца, баг ли это (нерабочая настройка окружения для этих двух переменных) или совпадение, что fallback-значения и так устраивают продакшен.
4. **`WS_SERVER_INTERNAL_URL`**, задаваемая в `docker-compose.yml:20` для `perricheno-site`, не встречается ни в одном файле `src/` — похоже на неиспользуемую или задел-на-будущее переменную.
...
6. **`.github/workflows/deploy.yml` — `KASPI_MERCHANT_ID`/`KASPI_API_KEY`/`KASPI_WEBHOOK_SECRET`** используются в серверном скрипте (строки 80-82), но не объявлены в `env:`/`envs:` шага (строки 32-49), поэтому эти три `update_env`-вызова на практике не должны ничего записывать через данный пайплайн CI/CD — переменные пришлось бы прописывать в серверный `.env` вручную. Комментарий в файле говорит "optional — set in GitHub secrets when ready", но стоит подтвердить: это известное временное состояние (фича Kaspi ещё не активирована в CI) или недосмотр.
7. **Повтор одного и того же несекретного по факту креденшла MinIO в 4 местах** (`docker-compose.yml` дважды, `.github/workflows/deploy.yml`, `src/lib/storage.ts` как fallback) — не проблема сама по себе (значение и так закоммичено открытым текстом), но стоит уточнить у владельца, не планируется ли вынести его в единственный источник правды (например, только в `.env`, без литералов в остальных файлах), особенно если когда-нибудь потребуется сменить этот пароль.
8. **`get_bot_name.js`** в корне репозитория — одноразовый dev-скрипт с захардкоженным токеном бота, не использующий `process.env.TELEGRAM_BOT_TOKEN`. Стоит уточнить, актуален ли он ещё (используется ли реально этот номер токена сейчас) и не стоит ли удалить файл или хотя бы параметризовать его переменной окружения, раз он лежит в общем репозитории.
