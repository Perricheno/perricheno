...
| `LATEX_COMPILER_URL` | `agent/compile-pdf/route.ts:8`, `tikz/generate/route.ts:11` (через `!` non-null assertion — TS считает переменную обязательной, но рантайм-проверки нет), `space/[id]/compile/route.ts:9`, `src/lib/agent/pipeline/stage5_validate.ts:14`, `src/lib/agent/pipeline/stage2_3_visuals.ts:12` (тоже `!`), `src/lib/receiptGenerator.ts:110`, `src/app/[locale]/dashboard/actions.ts:107` (только `GET ${url}/health`, health-check, без авторизации — единственное место с другим путём), `internal/bot/history/files/route.ts:122` | **[ИЗМЕНЕНО, Фаза 6a-latex, 2026-09-04]**: раньше — URL внешнего компилятора LaTeX→PDF, код и настройка которого нигде не хранились (терялись при каждой пересборке инфраструктуры). Теперь — собственный сервис в этом репозитории (`latex-compiler/`, Go + Gin + полный TeX Live), в `docker-compose.yml` фиксировано `http://latex-compiler:8080`. API-контракт воспроизведён по фактическому использованию во всех перечисленных файлах (POST без пути, `multipart/form-data` с полем `file` — zip проекта, заголовки `x-api-key`/`X-Main-File`/`X-Compiler`) — клиентский код не менялся. | Больше не секрет — фиксированное внутреннее значение в `docker-compose.yml`, не форвардится через GitHub Secrets (убрано из `deploy.yml` в этой же фазе). |
| `LATEX_COMPILER_KEY` | Парой с `LATEX_COMPILER_URL` во всех перечисленных выше файлах (кроме health-check в `dashboard/actions.ts`, который вообще не шлёт этот заголовок) | API-ключ авторизации к сервису `latex-compiler` (проверяется в `latex-compiler/main.go` через `x-api-key`). | **REQUIRED** — `latex-compiler/main.go` использует тот же `mustEnv()`-паттерн, что `pdf-extractor`/`telegram-bot`: падает при старте, если не задан. Секрет, форвардится через GitHub Secrets/`deploy.yml` как и раньше (значение сгенерировано заново вместе с новым сервисом — старое, для внешнего компилятора, более не действует). |
...
| Переменная | Где используется | Назначение | Обязательность |
|---|---|---|---|
| `REDIS_URL` | `src/lib/queue.ts:16-19` — читается и продюсером (`perricheno-site`, вызывает `getQueue().add(...)` из `route.ts`), и консьюмером (`worker`, вызывает `getRedisConnection()` в `src/worker/index.ts`) | Строка подключения к Redis (очередь BullMQ), включает пароль: `redis://:${REDIS_PASSWORD}@redis:6379`. | **REQUIRED, но лениво** — `getRedisConnection()`/`getQueue()` бросают `throw` только при первом реальном обращении к очереди (не при старте процесса), поэтому обычные HTTP-роуты, не трогающие генерацию, продолжат работать даже без неё; но `worker` не запустится вообще без переменной, так как создаёт подключение сразу при старте. **[Уточнено при реальном деплое, 2026-09-04]**: пароль обязателен не только для безопасности, но и функционально — Redis 7 по умолчанию включает `protected-mode`, который отклоняет любое подключение не по loopback, если пароль не задан (подтверждено вживую: `perricheno-worker` получал `NOAUTH` при подключении по докер-сети, хотя `redis-cli ping` изнутри контейнера редиса по loopback проходил без пароля). |
| `REDIS_PASSWORD` | `docker-compose.yml` — `redis`-сервис (`--requirepass`, healthcheck), собирается в `REDIS_URL` для `perricheno-site`/`worker` | Пароль Redis. | **REQUIRED** — `${REDIS_PASSWORD:?REDIS_PASSWORD is required}` в трёх местах `docker-compose.yml` (команда контейнера `redis`, его healthcheck, и `REDIS_URL` двух потребителей). |
...
- **`.github/workflows/deploy.yml`** прокидывает секреты GitHub Actions (`secrets.*`) в SSH-сессию на сервере и дописывает/обновляет ими файл `.env` через bash-функцию `update_env`. В `env:`/`envs:` блоке шага явно перечислены и форвардятся: `OPENAI_API_KEY`, `LATEX_COMPILER_KEY`, `CRYPTOCLOUD_API_KEY`, `CRYPTOCLOUD_SHOP_ID`, `CRYPTOCLOUD_SECRET`, `TELEGRAM_BOT_TOKEN`, `WEBHOOK_DOMAIN`, `WEBHOOK_SECRET`, `WS_JWT_SECRET`, `NEXT_PUBLIC_WS_URL`, `SESSION_SECRET`, а с Фазы 1 (2026-09-03) также `STIRLING_PDF_API_KEY`, `PADDLEOCR_API_TOKEN`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`, `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`, с Фазы 6a (2026-09-04) — `REDIS_PASSWORD`. **[УДАЛЕНО, Фаза 6a-latex, 2026-09-04]**: `LATEX_COMPILER_URL` больше не форвардится — это фиксированное внутреннее значение в `docker-compose.yml` (`http://latex-compiler:8080`), не секрет.
...
2. ~~`LATEX_COMPILER_URL` fallback `http://latex-compiler:8000`~~ — **[ИСПРАВЛЕНО, Фаза 2, 2026-09-03]**: владелец не подтвердил реальную инфраструктуру за этим хостом, поэтому мёртвый фолбэк убран (не гадаю про несуществующий сервис) — оба места (`receiptGenerator.ts`, `dashboard/actions.ts`) теперь явно проверяют конфиг и сообщают об ошибке вместо попытки достучаться до нерезолвящегося хоста. **[Фаза 6a-latex, 2026-09-04]**: хост `latex-compiler` теперь реально существует в `docker-compose.yml` (порт 8080, не 8000) — собственный сервис вместо внешнего.
