...
> ⚠️ **Важное расхождение (см. также `docs/REVIEW.md`)**: в репозитории есть ещё два набора «сырых» SQL-миграций — `/migrations/*.sql` и `/supabase/migrations/*.sql`. Они описывают **другую** схему: таблицы в lowercase (`citations`, `spaces`, `space_files`, `users`), Postgres-триггеры для `updated_at`, GIN-индексы по тегам и Supabase Storage RLS-политики на `auth.uid()`. Код приложения (`src/lib/citations-db.ts`, `src/lib/space-db.ts` и т.д.) обращается к данным исключительно через Prisma Client (`prisma.citation.findMany(...)`), то есть работает с PascalCase-таблицами из `schema.prisma`, а не с теми, что создали бы эти SQL-файлы. Похоже, это наследие более раннего этапа (SQLite → Supabase → self-hosted Postgres, см. `/scripts/migrate-*`), которое осталось в репозитории неприменённым/устаревшим. **Требует уточнения у владельца**: применялись ли эти SQL-файлы когда-либо к текущей продовой БД, и можно ли их удалить, чтобы не вводить в заблуждение.
...
### Citation
Одна библиографическая запись: идентификаторы `doi`/`arxiv_id`/`isbn` (все опциональны, не проверяется что хотя бы один задан на уровне схемы), `authors`/`tags` — нативные Postgres-массивы строк (`String[]`), `bibtex` (готовый BibTeX-текст) + `cite_key` (ключ вида `@article{KEY,...}`). Никакого `@@unique` на `(user_id, cite_key)` в Prisma-схеме нет (в отличие от "мёртвой" SQL-версии в supabase/migrations, где такой constraint был) — то есть в текущей рабочей схеме два cite_key у одного пользователя технически могут совпасть.
