...
### 2.10 `account_tier` vs `plan_tier` — [ИСПРАВЛЕНО, Фаза 4, 2026-09-04]

Ранее в `User` было **два** похожих поля: `account_tier` (default `'free'`) и `plan_tier` (default `'free'`). Расследование подтвердило реальный баг, а не просто стилистический дубль:
- `upgradeSubscriptionPlan()` (db.ts, вызывается из вебхуков оплаты) обновляла **оба** поля одинаково.
- `checkAndDeductUsage()` и лимит стейджинга PDF (`PDF_STAGING_CAPS`) читали **только `plan_tier`** — то есть это было единственное поле, реально управлявшее квотой.
- Административная ручка `set_tier` в `src/app/api/admin/users/route.ts` меняла **только `account_tier`**.
- Бот-роут `src/app/api/internal/bot/billing/route.ts` отображал лимиты по **`account_tier`**.

Т.е. если админ через дашборд менял тариф пользователю вручную (`set_tier`), реальная квота (`plan_tier`-based) не менялась, а в боте пользователь видел новый тариф с ЛИМИТАМИ, которые фактически не применялись при генерации.

**Исправление**: `account_tier` удалён полностью — поле схемы, обе записи в `upgradeSubscriptionPlan()`, `set_tier` и `internal/bot/billing` теперь читают/пишут `plan_tier`, клиентские отображения (`LoginModal.tsx`, `AgentBillingModal.tsx`) и тип `src/types/user.ts` обновлены. Единственное поле тарифа — `plan_tier`. См. §10 в `REVIEW.md` про риск для уже разошедшихся данных на проде при следующем `db push`.
...
8. ~~**[НЕ защищён]** «`account_tier` и `plan_tier` синхронизированы»~~ — **[ИСПРАВЛЕНО, Фаза 4, 2026-09-04]**: `account_tier` удалён полностью, осталось единственное поле `plan_tier`, которое теперь обновляют все пути (платёжный `upgradeSubscriptionPlan()` и админский `set_tier`). См. §2.10.
