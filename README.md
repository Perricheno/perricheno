...
**В проекте нет ни одного тестового файла** (проверено `Glob` по `**/*.test.*` — 0 совпадений). `npm run lint` и `npx tsc --noEmit` — единственные автоматические проверки, и лишь вторая реально выполняется в CI. Подробности и что стоит покрыть в первую очередь (деньги/авторизация/необратимые операции) — `docs/REVIEW.md`.
