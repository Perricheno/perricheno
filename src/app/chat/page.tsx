...
import {
    ...
    IconLogin, IconSun, IconMoon, IconMenu2, IconRobot
...
export default function ChatPage() {
        ...
        try {
            ...
            const payload = {
                ...
                context: {
                    ...
                    previousMessages: messages.slice(-5).map(m => ({ role: m.role, text: m.text })), // Send last 5 for context
                },
                ...
                // Files handled separately via FormData if needed, but here we can send metadata
            ...
            // We use the proxy to forward to n8n to avoid CORS issues if n8n not configured
            // Or send directly if n8n supports it. Using proxy is safer for secrets.
            const response = await fetch(webhookUrl || "/api/webhook-proxy", { // Fallback if no URL
                method: "POST",
                body: formData, // Sending FormData to support file uploads
            });

            if (!response.ok) throw new Error(`Webhook Error: ${response.status}`);

            const data = await response.json();
            // Expecting n8n to return JSON with 'output' or 'text'
    ...
    }, [input, files, isLoading, settings, messages, activeThreadId, user, activeThread]);
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] pl-16 md:pl-64 flex overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col h-screen min-w-0 bg-[var(--background)] relative">
                ...
                {/* Input Area */}
                <div className="p-4 md:p-6 bg-[var(--background)] relative z-20">
                    <div className="max-w-4xl mx-auto border border-[var(--border)] rounded-2xl bg-[var(--background)] shadow-sm transition-all focus-within:shadow-md focus-within:border-[var(--foreground)]">
                        ...
                        <div className="flex items-end gap-2 p-2">
                             <button onClick={() => fileInputRef.current?.click()} className="p-3 text-[var(--foreground)] opacity-40 hover:opacity-100 hover:bg-[var(--muted)] rounded-xl transition-all">
                            ...
                            <textarea 
                                ...
                                placeholder={!user ? "Please sign in to chat..." : "Type your message..."}
                                disabled={isRecording || !user}
                                className="flex-1 bg-transparent max-h-[150px] py-3 px-1 outline-none resize-none text-sm md:text-base leading-relaxed"
                                rows={1}
                            />
                            ...
                            <div className="flex gap-1">
                                ...
                                <button onClick={() => sendMessage()} disabled={isLoading || !user || (!input.trim() && files.length === 0)} 
                                    className="p-3 bg-[var(--foreground)] text-[var(--background)] rounded-xl hover:opacity-90 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
